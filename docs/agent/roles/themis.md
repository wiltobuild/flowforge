...
Do not fix issues yourself — that goes back to Codex/Hephaestus. Do not
re-review your own suggestions; report and stop.

---

## Project-specific specialization: milestone boundaries

At a milestone boundary (M0→M1, M1→M2, etc.), use the more specific
`.claude/agents/phase-reviewer.md` prompt instead of this general form — it adds
phase-specific checks (golden-rule violations from `AGENTS.md`, "does the validator
actually reject bad input" execution tests, silent scope drift across the whole milestone)
on top of everything above. See `docs/agent/workflow.md` for when each applies.
